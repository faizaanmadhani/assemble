...
import { convertToSourceTC, propertyToSourceGraphQLType, inputPropertiesToGraphQLTypes, getSubFields } from '../mappingConverter';
const mapping = {
  properties: {
    name: {
      type: 'text',
...
describe('PropertiesConverter', () => {
  ...
  describe('propertyToSourceGraphQLType(schemaComposer, )', () => {
    ...
    it('should return GraphQLJSON as fallback for unknown Elastic type', () => {
      expect(propertyToSourceGraphQLType(schemaComposer, {
        type: 'strange'
      })).toEqual('JSON');
    });
    ...
    it('should return GraphQLObjectType for object with subfields', () => {
      const tc = propertyToSourceGraphQLType(schemaComposer, {
        properties: {
          big: {
            type: 'text'
          },
  ...
  describe('getSubFields()', () => {
    it('should return array of sub fields', () => {
      expect(getSubFields('range', ['ok', 'range', 'range.min', 'range.max'])).toEqual(['min', 'max']);
    });
  ...
  describe('issue #9', () => {
    ...
    it('should use Elastic field names from source', async () => {
      ...
      const result = await graphql.graphql(schemaComposer.buildSchema(), `
          query {
            userES {
              _id
              lastName
