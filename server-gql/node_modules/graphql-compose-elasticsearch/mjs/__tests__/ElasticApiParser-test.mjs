  ...
  describe('static methods', () => {
    describe('loadApiFile()', () => {
      ...
      it('should replace invalid markup', () => {
        expect(ElasticApiParser.loadApiFile(apiPartialPath)).not.toContain('@param {<<api-param-type-string,`String`>>} params.analyzer - The analyzer to use for the query string');
        expect(ElasticApiParser.loadApiFile(apiPartialPath)).toContain('@param {String} params.analyzer - The analyzer to use for the query string');
      });
    });
    describe('findApiVersionFile()', () => {
      it('should find proper version in elasticsearch 12.x', () => {
        ...
        expect(ElasticApiParser.findApiVersionFile('5.0')).toMatch('elasticsearch/src/lib/apis/5_0.js');
        expect(ElasticApiParser.findApiVersionFile('2.4')).toMatch('elasticsearch/src/lib/apis/2_4.js');
        expect(ElasticApiParser.findApiVersionFile('1.7')).toMatch('elasticsearch/src/lib/apis/1_7.js');
        expect(ElasticApiParser.findApiVersionFile('_default')).toMatch('elasticsearch/src/lib/apis/5_0.js'); // $FlowFixMe
      ...
      it('should find proper version in elasticsearch 13.x', () => {
        ...
        expect(ElasticApiParser.findApiVersionFile('2.4')).toMatch('elasticsearch/src/lib/apis/2_4.js');
        expect(ElasticApiParser.findApiVersionFile('1.7')).toMatch('elasticsearch/src/lib/apis/1_7.js');
        expect(ElasticApiParser.findApiVersionFile('_default')).toMatch('elasticsearch/src/lib/apis/5_3.js'); // $FlowFixMe
    ...
    describe('cleanUpSource()', () => {
      it('should {<<api-param-type-boolean,`Boolean`>>} convert to {Boolean}', () => {
        expect(ElasticApiParser.cleanUpSource(`@param {<<api-param-type-boolean,\`Boolean\`>>} params.analyzeWildcard`)).toEqual(`@param {Boolean} params.analyzeWildcard`);
      });
      it("should api['delete'] convert to api.delete", () => {
        expect(ElasticApiParser.cleanUpSource(`api.indices.prototype['delete'] = ca({`)).toEqual(`api.indices.prototype.delete = ca({`);
      });
    });
    describe('parseParamsDescription()', () => {
      it('should return descriptions for fields', () => {
        ...
        `);
        const doxAST = dox.parseComments(source, {
          raw: true
        });
    ...
    describe('codeToSettings()', () => {
      it('should return settings as object from ca({ settings })', () => {
        expect(ElasticApiParser.codeToSettings(`
    ...
    describe('getMethodName()', () => {
      ...
      it('should return array of string', () => {
        expect(ElasticApiParser.getMethodName('api.cat.prototype.allocation')).toEqual(['cat', 'allocation']);
      });
    });
    describe('parseSource()', () => {
      ...
      it('should return ElasticParsedSourceT', () => {
        expect(ElasticApiParser.parseSource(ElasticApiParser.loadApiFile(apiPartialPath))).toMatchSnapshot();
      });
    });
  });
  ...
  describe('paramToGraphQLArgConfig()', () => {
    ...
    it('should ignore non-numeric default for float', () => {
      expect(parser.paramToGraphQLArgConfig({
        type: 'number',
        default: 'ABC'
      }, 'someField')).not.toHaveProperty('defaultValue');
    });
    it('should accept numeric default for float', () => {
      expect(parser.paramToGraphQLArgConfig({
        type: 'number',
        default: '4.2'
      }, 'someField')).toHaveProperty('defaultValue', 4.2);
    });
  });
